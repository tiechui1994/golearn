...
该桶的第7,8位cell还未有对应的键值对. 注意: key和value是各自存储起来的, 并非想象中的 key/value/key/value...的形式. 
这样的做法好处在于消除key/value之间的padding, 例如`map[int64]int`. 还有,在8个键值对数据后面还有一个overflow指针, 
因为桶中最多只能装8个键值对, 如果有多余的键值对落到当前桶, 那么就需要再构建一个桶(溢出桶), 通过 overflow 指针链接起来.
...
对于不指定初始化大小, 和初始化值 hint < 8 (bucketCnt) 时, go会调用 `make_small` 函数, 并直接从堆上进行分配.
...
> 注: 上述的 makeBucketArray 代码当前可以得到的一个结论是, **正常状况下**, 正常的桶和溢出的桶在内存存储空间是连续的, 
...
> 注: `mapassign` 的代码当中, 没有直接将 value 写入内存, 而是将 value 在内存当中的对应地址返回, 后续对内存地址写入
进行操作. 
...
> 总结: 对于第2点, 是对第1点的补充. 因为在装载因子比较小的状况下, 有可能 map 的查找和插入效率也很低, 而第1点识别不出
来的这种状况. 表面现象就是计算装载因子的分子比较小, 即map里的元素总数少, 但是桶的数量多(包含了大量的溢出桶).

针对上述两种状况, 采用不同的解决方案:

针对1, 将 B+1, 新建一个buckets数组, 新的buckets大小是原来的2倍, 然后旧的buckets数据搬移到新的buckets. 该方法称为
增量扩容.

针对2, 并不扩容, bucket数量维持不变, 重新做一遍类似增量扩容的搬移动作. 把松散的键值对重新排列一次, 以使得bucket的使用
率更高, 进而保证更快的存取. 该方法称为等量扩容.


对于2的解决方案, 存在一个极端情况: 如果插入 map 的 key 哈希都一样, 那么它们就会落到同一个 bucket 里, 超过8个就会产生
overflow bucket, 最终还是会造成overflow bucket过多. 移动元素其实解决不了问题. 但是 Go 的每一个 map 都会在初始化
阶段 makemap 时产生一个随机的哈希种子, 所以构造这种冲突没那么容易.

扩容主要涉及 `hashGrow()` 和 `growWork()` 两个函数. `hashGrow()` 是开启扩容, 并没有进行真正的数据"搬移",它只是分
配好了新的 buckets, 并将老的 buckets 赋值给 oldbuckets字段上. 真正的 buckets 搬移动作是在 `growWork()` 函数.调
用 `mapassign()`(插入,更新) 和 `mapdelete()`(删除) 函数时会尝试搬移 buckets 的工作.
...
// 扩容, 启动map扩容操作. 只是完成了hmap 元数据的复制, 但是底层的 bucket 没有进行搬移.
```cgo
...
func evacuate(t *maptype, h *hmap, oldbucket uintptr) {
  ...
  if !evacuated(b) {
    ...
    // evacuate 函数每次只完成一个 bucket 的搬迁工作, 因此要遍历完此 bucket 的所有的 cell, 
    // 将有值的 cell copy 到新的地方.
      ...
      for i := 0; i < bucketCnt; i, k, e = i+1, add(k, uintptr(t.keysize)), add(e, uintptr(t.elemsize)) {
		...
		if !h.sameSizeGrow() {
		  ...
		  if h.flags&iterator != 0 && !t.reflexivekey() && !t.key.equal(k2, k2) {
			...
			// 当它作为 map 的 key时, 会遇到一个问题: 再次计算它的哈希值和它当初插入 map 时的
			// 计算出来的哈希值不一样! 这个 key 是永远不会被 Get 操作获取的! 当使用 m[math.NaN()] 
			// 语句的时候, 是查不出来结果的. 这个 key 只有在遍历整个 map 的时候, 才能被找到.
		...
		// useY要么为0, 要么为1. 这里就是选取在bucket x的起始内存位置, 或者选择在bucket y
		// 的起始内存位置 (只有增量同步才会有这个选择可能).
		dst := &xy[useY]
		...
		// 计算下一个k, e的内存地址
		// 由于桶的内存布局中在最后还有overflow的指针, 所以这里不用担心更新有可能会超出
		//key和value数组的指针地址.
...
func advanceEvacuationMark(h *hmap, t *maptype, newbit uintptr) {
  ...
  if h.nevacuate == newbit {
    ...
    // 所以, 这种情况下, 其实我们是搬迁的extra的buckets数组. 因此, 在这种情况下, 
    // 需要在搬迁完毕后, 将 hmap.extra.oldoverflow指针置为nil.
    if h.extra != nil {
...
增量扩容下 map 搬移的图解, 其中 B=3, oldbuckets 数组是8. buckets数组是16

![image](/images/develop_map_growth_one.png) 
...
func mapdelete(t *maptype, h *hmap, key unsafe.Pointer) {
  ...
  // 当h.flags对应的值为hashWriting (代表有其他goroutine正在往map中写key)时, 
  // 那么位计算的结果不为0, 因此抛出以下错误.
  if h.flags&hashWriting != 0 {
    throw("concurrent map writes")
  }
  ...
  // 调用t.hasher后设置hashWriting, 因为t.hasher可能会 panic, 在这种情况下, 我们
  // 实际上并没有执行写(删除)操作.
  h.flags ^= hashWriting
  ...
  // 第二种是当前桶再无溢出桶;
  // 第三种是当前桶中有cell位的tophash值是emptyRest, 这个值在前面解释过, 它代表
  // 此时的桶后面的cell还未利用,所以无需再继续遍历. 
search:
  ...
  for ; b != nil; b = b.overflow(t) {
    for i := uintptr(0); i < bucketCnt; i++ {
      ...
      // 可以立即结束循环的的两种状况:
      // 情况1: 当前 cell 是 bucket 的最有一个 cell, 且后续的 overflow bucket 
      // 的 cell tophash 不为 emptyRest
      ...
      for {
		...
		if i == 0 {
		  ...
		  // 获取当前 bucket 的前面的 prev bucket(即 prev bucket 的 overflow 是当前 
		  // bucket) 每次都是从桶内的首个元素开始
		  c := b
...
func mapiterinit(t *maptype, h *hmap, it *hiter) {
  ...
  // hiter 的大小是 12 个系统指针大小. 在 cmd/compile/internal/gc/reflect.go:hiter() 
  // 当中有这样的体现
  ...
  if t.bucket.ptrdata == 0 {
    ...
    // 这样在迭代的过程中, 可以让 overflow bucket 处于活动状态, 不论 table 的增长 and/or 
    // 新的 overflow buckets 被添加到table当中
    h.createOverflow()
...
next:
  ...
  if b == nil {
    ...
    if h.growing() && it.B == h.B {
      ...
      // 如果我们要查看的存储桶尚未装满(即old bucket尚未搬移), 则我们需要遍历old bucket, 
      // 只返回将要迁移到该 bucket 的 cell.
  ...
  for ; i < bucketCnt; i++ {
    ...
    if checkBucket != noCheck && !h.sameSizeGrow() {
      ...
      // 因此, 正在遍历oldbucket, 需要跳过将要转到另一个新bucket的所有键 (在增长过程中, 
      // 每个oldbucket会扩展为两个bucket).
      // 
      // reflexivekey() // true if k==k for all keys
    ...
    if (b.tophash[offi] != evacuatedX && b.tophash[offi] != evacuatedY) ||
      ...
      // 或者
      // key != key (只能发生 key=NANs 的状况下), 这些key是没法更新和删除的, 
      // 只能在遍历的时候返回.
      it.key = k
      if t.indirectelem() {
		e = *((*unsafe.Pointer)(e))
