...
```
dp[i][k][s], i 表示天数, k 表示股票交易次数, s 表示当前的状态(1:持有, 0:不持有)
```

股票只能买卖一次最大收益:

```
dp[i][1][0] = max(dp[i-1][1][0], dp[i-1][1][1] + prices[i]) // 卖出
dp[i][1][1] = max(dp[i-1][1][1], dp[i-1][0][0] - prices[i]) // 买入

由于 dp[i-1][0][0] = 0, 因为交易次数是 0
