...
> 注: NULL 是一个宏变量, 定义在头文件 `<stdlib.h>` 当中, 在Go当中可以使用 `C.NULL` 获取其值(0)

在 Go 语言当中, 指针类型的控制为 `nil`. 

在 Go 调用 C 的时候, 如果函数的某个参数需要传值为 `NULL` 或某个参数内的成员变量为指针类型, 且此时需要传值为 `NULL`,
这个时候该怎么解决呢? 对于后一种情况, 很简单 `成员变量不要赋值即可`. 但是前一种状况, 是必须要有值传递的, 此时传递的值应
该是 `(*C.char)(unsafe.Pointer(nil))` (这里是以 `char*` 类型为例). 
