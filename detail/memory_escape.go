...
/*
1. channel 发送的值是 "指针" 或 "其包含指针", 由于在编译阶段无法确定其作用域与传递的路径, 所以
一般都会逃逸到堆上分配(指针部分). 注意, channel本身的底层存储结构没有逃逸.

2. slices 中的值是指针的指针或包含指针字段. 一个例子是类似[]*string 的类型. 这总是导致slice的
逃逸. 即使切片的底层存储数组仍可能位于堆栈上, 数据的引用也会转移到堆中.

3. slice 由于 append 操作超出其容量, 因此会导致 slice 重新分配. 这种情况下, 由于在编译时 slice
的初始大小的已知情况下, 将会在栈上分配. 如果 slice 的底层存储必须基于仅在运行时数据进行扩展, 则它
将分配在堆上.

4. 调用接口类型的方法. 接口类型的方法调用是动态调度 - 实际使用的具体实现只能在运行时确定. 考虑一个
接口类型为io.Reader 的变量 r. 对 r.Read(b) 的调用将导致 r 的值和字节片 b 的后续转义并因此分配
到堆上.
