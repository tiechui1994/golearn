...
range 操作进行值复制的操作. 属于浅拷贝(被遍历的值, 遍历过程中的元素). 浅拷贝可能会修改原数据.
...
func ArrayRange() {
	...
	// range a1, 就是一个值拷贝过程. a1 是一个 array, 值拷贝实际上就是深拷贝.
	for i, v := range a1 {
		if i == 0 {
			a1[1] = 11
			a1[2] = 22
...
func SliceRange() {
	...
	// range a1 是一个值拷贝, 由于 a1 是 slice, 相当于指针拷贝.
	for i, v := range a1 {
		if i == 0 {
			a1[1] = 11
			a1[2] = 22
...
type A struct{}
type B = A  // B是A的别名, A和B是同一种类型, 无法再对 B 增加新的方法.
type C A    // 创建新的结构体C, A和C是两种类型, 可以对 C 增加新的方法.
